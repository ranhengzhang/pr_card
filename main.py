...
from src.format_utils import generate_output_filename
...
def resolve_repo(args: argparse.Namespace) -> Tuple[str, str]:
    ...
    if args.url:
        ...
        with GitHubFetcher() as fetcher:
            parts = fetcher.parse_pr_url(args.url)
...
async def generate_card_from_url(
    ...
    with GitHubFetcher(token) as fetcher:
        ...
        parts = fetcher.parse_pr_url(url)
        owner, repo_name, pr_number = parts["owner"], parts["repo"], parts["number"]
        logger.info("正在获取 PR 评论...")
